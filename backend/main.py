...
from backend.reporting.excel_output import save_workbook
...
def upload_master_data(file: UploadFile = File(...)):
    path = _save_upload(file, "master_data")
    try:
        result = ingest_master_data(path)
...
def upload_abnormal_shortage(file: UploadFile = File(...)):
    path = _save_upload(file, "abnormal_shortage")
    try:
        counts, names = ingest_abnormal_shortage(path, state.current_cycle_fy)
    except Exception as e:
...
def upload_fake_documents(file: UploadFile = File(...)):
    path = _save_upload(file, "fake_documents")
    try:
        counts, names = ingest_fake_documents(path, state.current_cycle_fy)
    except Exception as e:
...
def upload_seal_tempering(file: UploadFile = File(...)):
    path = _save_upload(file, "seal_tempering")
    try:
        counts, names = ingest_seal_tempering(path)
    except Exception as e:
...
def upload_quality_fail(
    ...
    unauth_path = _save_upload(unauthorized_mod, "unauthorized_mod")
    reversal_path = _save_upload(reversal_cases, "reversal_cases")
    try:
        counts, names = ingest_quality_fail(unauth_path, reversal_path, state.current_cycle_fy)
    except Exception as e:
...
def upload_contractor_emails(body: ContractorEmailsBody):
    """Persisted to data/contractor_emails.json so the address book survives
    a server restart, not just the in-memory run — set once, reused every
    evaluation cycle. An empty-string email removes that contractor's entry."""
...
def calculate():
    ...
    save_workbook(rows, state.scores, output_path, state.previous_scores)
...
def get_historical():
    """Reads every archived per-period workbook in data/output/history back
    off disk and returns per-contractor, per-category scores so the
    Historical Performance screen can chart trends across periods/browsers."""
    manifest = load_history_manifest()
    periods = []
...
def preview_report(cc_number: int):
    ...
    path = generate_report(cc_number, name, score, state.period_label, REPORTS_DIR)
    state.generated_reports[cc_number] = path
    return FileResponse(
        path,
...
def send_reports(body: SendReportsBody):
    ...
    for cc in body.cc_numbers:
        ...
        report_path = state.generated_reports.get(cc) or generate_report(cc, name, score, state.period_label, REPORTS_DIR)
        state.generated_reports[cc] = report_path
